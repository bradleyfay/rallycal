...
class EventDeduplicator:
    ...
    def find_duplicates(
        ...
        for event in events:
            ...
            for candidate in potential_duplicates:
                ...
                similarity_factors = self._calculate_similarity_factors(
                    event, candidate
                )
    ...
    def _calculate_similarity_factors(
        ...
        # Additional factors
        factors["source_difference"] = (
            0.0 if event1.source_name == event2.source_name else 1.0
        )
        factors["event_type_match"] = (
            1.0 if event1.event_type == event2.event_type else 0.0
        )
    ...
    def _apply_sports_specific_matching(
        ...
        has_game_pattern1 = any(
            re.search(pattern, text1.lower()) for pattern in game_patterns
        )
        has_game_pattern2 = any(
            re.search(pattern, text2.lower()) for pattern in game_patterns
        )
    ...
    def _extract_team_names(self, text: str) -> set[str]:
        ...
        if vs_match:
            ...
            # Clean up team names
            team1 = re.sub(
                r"^.*?(eagles|tigers|lions|bears|wolves|sharks|hawks|falcons|panthers|bulls|rams|saints|giants|jets|raiders|chiefs|broncos|steelers|cowboys|patriots|dolphins|bills|cardinals|vikings|packers|49ers|seahawks|chargers|browns|ravens|bengals|titans|jaguars|colts|texans|redskins).*?$",
                r"\1",
                team1,
                flags=re.IGNORECASE,
            )
    ...
    def _calculate_time_similarity(
        ...
        start_similarity = max(
            0.0, 1.0 - (start_diff.total_seconds() / max_time_diff.total_seconds())
        )
        ...
        duration_similarity = max(
            0.0,
            1.0 - (duration_diff.total_seconds() / max_duration_diff.total_seconds()),
        )
    ...
    def merge_duplicate_events(
        ...
        if duplicate_event.location and not canonical_event.location:
            ...
            duplicate_event.location
            and canonical_event.location
            and len(duplicate_event.location) > len(canonical_event.location)
        ):
            merged_event.location = duplicate_event.location
