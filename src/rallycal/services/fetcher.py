...
class CalendarFetcher:
    ...
    async def fetch_calendar(
        ...
        try:
            ...
            if response.status_code == 401:
                raise AuthenticationError(f"Authentication failed for {calendar_source.name}")
            ...
            if not self._is_valid_calendar_content(calendar_data):
                raise ParsingError(f"Invalid calendar content from {calendar_source.name}")
            ...
            raise CalendarFetchError(f"Request failed for {calendar_source.name}") from e
        
        except Exception as e:
            logger.error(f"Unexpected error fetching calendar {calendar_source.name}: {e}")
            raise CalendarFetchError(f"Unexpected error for {calendar_source.name}") from e
    ...
    def _detect_event_type(
        ...
        type_keywords = {
            ...
            EventType.PRACTICE: ["practice", "training", "drill", "scrimmage", "workout"],
            EventType.TOURNAMENT: ["tournament", "championship", "cup", "classic", "invitational"],
    ...
    async def fetch_multiple_calendars(
        ...
        successful_fetches = sum(1 for events, error in result_dict.values() if error is None)
        logger.info(
