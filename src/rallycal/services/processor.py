...
class TitleFormatter:
    ...
    def format_title(
        ...
        if (self.include_source_labels and 
            event.source_name and 
            not self._has_source_label(event.title, event.source_name)):
        ...
        if (self.include_event_type and 
            event.event_type and 
            not self._has_type_indicator(clean_title)):
            type_label = self.type_label_format.format(type=event.event_type.value.title())
...
class EventProcessor:
    ...
    async def process_events(
        ...
        for event in new_events:
            ...
            is_duplicate = duplicate_results.get(event_id, {}).is_duplicate if duplicate_results else False
    ...
    def _extract_team_name(self, title: str) -> str | None:
        ...
        vs_match = re.search(r'(\w+)\s+(?:vs?|versus|@|at)\s+(\w+)', title, re.IGNORECASE)
        if vs_match:
            # Return the first team mentioned
            return vs_match.group(1).lower()
        
        # Look for team name patterns
        team_pattern = re.search(r'\b(eagles|tigers|lions|bears|wolves|sharks|hawks|falcons|panthers|bulls|rams)\b', title, re.IGNORECASE)
        if team_pattern:
