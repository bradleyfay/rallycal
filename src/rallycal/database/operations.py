...
from ..models.event import EventModel as EventPydantic, EventStatus, EventType
from .models import (
    Base,
    CalendarSourceModel,
    ConfigurationModel,
...
class CalendarSourceRepository(BaseRepository[CalendarSourceModel]):
    ...
    async def get_due_for_sync(self, max_age_seconds: int = 3600) -> Sequence[CalendarSourceModel]:
    ...
    async def get_with_recent_sync_history(
        ...
        query = (
            select(CalendarSourceModel)
            .options(
                selectinload(CalendarSourceModel.sync_history).limit(limit)
            )
            .order_by(CalendarSourceModel.name)
        )
...
class SyncHistoryRepository(BaseRepository[SyncHistoryModel]):
    ...
    async def get_recent_syncs(
        ...
        query = (
            select(SyncHistoryModel)
            .where(SyncHistoryModel.started_at >= cutoff_time)
        )
    ...
    async def cleanup_old_history(
        ...
        """
        cutoff_date = datetime.now(timezone.utc) - datetime.timedelta(days=retention_days)
        
        query = delete(SyncHistoryModel).where(SyncHistoryModel.started_at < cutoff_date)
        result = await self.session.execute(query)
        return result.rowcount
