...
class ICalGenerator:
    ...
    def __init__(
        ...
        self.description = (
            description or "Aggregated sports calendar from multiple sources"
        )
    ...
    def _add_timezone_component(self, cal: Calendar) -> None:
        ...
        for tz_name in timezones_needed:
            try:
                ...
                logger.warning(
                    f"Failed to create timezone component for {tz_name}: {e}"
                )
            ...
            if not dst_transitions:
                ...
                for transition in dst_transitions:
                    if transition["is_dst"]:
                        ...
                        daylight.add(
                            "tzoffsetfrom",
                            self._format_offset(transition["offset_from"]),
                        )
                        daylight.add(
                            "tzoffsetto", self._format_offset(transition["offset_to"])
                        )
    ...
    def _get_dst_transitions(
        self, tz_info: zoneinfo.ZoneInfo, year: int
    ) -> list[dict[str, Any]]:
            ...
            for month in range(1, 13):
                for day in [1, 8, 15, 22]:
                    try:
                        ...
                        prev_dt = (
                            dt.replace(day=day - 1)
                            if day > 1
                            else dt.replace(month=month - 1, day=28)
                            if month > 1
                            else dt.replace(year=year - 1, month=12, day=31)
                        )
    ...
    def _add_event_to_calendar(self, cal: Calendar, event: EventModel) -> None:
        ...
        if hasattr(event, "last_modified") and event.last_modified:
            ...
            ical_event.add("last-modified", datetime.now(UTC))
    ...
    def _generate_event_uid(self, event: EventModel) -> str:
        ...
        uid_content = (
            f"{event.title}{event.start_time}{event.end_time}{event.source_name}"
        )
    ...
    def validate_calendar(self, calendar_content: str) -> tuple[bool, list[str]]:
        ...
        if "CALSCALE" in cal_component:
            ...
            if calscale != "GREGORIAN":
                errors.append(
                    f"CALSCALE must be GREGORIAN if present, found: {calscale}"
                )
        ...
        for component in cal.walk():
            if component.name == "VEVENT":
                ...
                if has_dtstart and not (has_dtend or has_duration):
                    errors.append(
                        f"Event {event_count}: Must have either DTEND or DURATION with DTSTART"
                    )

                if has_dtend and has_duration:
                    errors.append(
                        f"Event {event_count}: Cannot have both DTEND and DURATION"
                    )
                ...
                if "UID" in component:
                    ...
                    if not uid or "@" not in uid:
                        errors.append(
                            f"Event {event_count}: UID should contain '@' for uniqueness"
                        )
                ...
                if "SUMMARY" in component:
                    ...
                    if len(summary) > 255:
                        errors.append(
                            f"Event {event_count}: SUMMARY longer than 255 characters"
                        )
                ...
                if "SEQUENCE" in component:
                    try:
                        ...
                        if sequence < 0:
                            errors.append(
                                f"Event {event_count}: SEQUENCE must be non-negative"
                            )
            ...
            if component.name == "VTIMEZONE":
                ...
                for sub_component in component.subcomponents:
                    if sub_component.name == "STANDARD":
                        ...
                        std_errors = self._validate_timezone_component(
                            sub_component, "STANDARD"
                        )
                        errors.extend(std_errors)
                    elif sub_component.name == "DAYLIGHT":
                        has_daylight = True
                        # Validate DAYLIGHT component
                        dst_errors = self._validate_timezone_component(
                            sub_component, "DAYLIGHT"
                        )
                        errors.extend(dst_errors)

                if not (has_standard or has_daylight):
                    errors.append(
                        "VTIMEZONE: Must have at least one STANDARD or DAYLIGHT component"
                    )
        ...
        for offset_prop in ["TZOFFSETFROM", "TZOFFSETTO"]:
            if offset_prop in component:
                ...
                if not self._validate_offset_format(offset_value):
                    errors.append(
                        f"{comp_type}: Invalid {offset_prop} format: {offset_value}"
                    )
    ...
    def _validate_structure(self, calendar_content: str) -> list[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("BEGIN:"):
                ...
                if not begin_stack:
                    errors.append(
                        f"Line {i + 1}: END:{component_type} without matching BEGIN"
                    )
                else:
                    begin_type, begin_line = begin_stack.pop()
                    if begin_type != component_type:
                        errors.append(
                            f"Line {i + 1}: END:{component_type} doesn't match BEGIN:{begin_type} on line {begin_line}"
                        )

        if begin_stack:
            for component_type, line_num in begin_stack:
                errors.append(
                    f"Line {line_num}: BEGIN:{component_type} without matching END"
                )
    ...
    def _validate_line_lengths(self, calendar_content: str) -> list[str]:
        ...
        for i, line in enumerate(lines):
            ...
            if len(line.encode("utf-8")) > 75:
                errors.append(
                    f"Line {i + 1}: Exceeds 75 octets ({len(line.encode('utf-8'))} octets)"
                )
